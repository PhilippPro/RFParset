...
# Example
regis = makeExperimentRegistry(id = "clas_nodesize", packages="randomForest")
...
pars = list(j = 1:nrow(clas))
task.design = makeDesign("taski", exhaustive = pars)
pars = list(ntree = c(ntree), nodesize = c(1:30))
forest.design = makeDesign("forest", exhaustive = pars)

addExperiments(regis, repls = 2, prob.designs = task.design, algo.designs = list(forest.design))
...
# Example
regis = makeExperimentRegistry(id = "reg_nodesize", packages="randomForest")
