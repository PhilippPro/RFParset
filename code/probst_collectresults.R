...
res_classif = reduceResultsDataTable(ids = ids_classif, fun = function(r) as.list(r), reg = regis, fill = TRUE)
...
res_regr = reduceResultsDataTable(ids = ids_regr, fun = function(r) as.list(r), reg = regis, fill = TRUE)
...
min(getJobStatus()$started, na.rm = T)
max(getJobStatus()$done, na.rm = T)
# ca. 4 Tage hat es gebraucht auf dem lokalen Rechner (12 Kerne)

error_ids = getErrorMessages()$job.id # errors nicht laden
ids_ok = c(1:114432)[-error_ids]
ids_classif = ids_ok[ids_ok %in% 1:71808]
ids_regr = ids_ok[ids_ok %in% 71809:114432]

res_classif_def = reduceResultsDataTable(ids = ids_classif, fun = function(r) as.list(r), reg = regis, fill = TRUE)

hyp_par_def = getJobTable()

res_regr_def = reduceResultsDataTable(ids = ids_regr, fun = function(r) as.list(r), reg = regis, fill = TRUE)

load("/nfsmb/koll/probst/Random_Forest/RFParset/results/results.RData")

save(res_classif, res_regr, hyp_par, res_classif_def, res_regr_def, hyp_par_def, file = "/nfsmb/koll/probst/Random_Forest/RFParset/results/results.RData")
